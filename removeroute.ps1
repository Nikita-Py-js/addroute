...
function Select-CfgFile {
    ...
    do {
        ...
        if (-not $isValid) {
            Write-Host "Некорректный ввод. Пожалуйста, введите число от 0 до $($cfgFiles.Count-1)."
        }
    } while (-not $isValid)
...
foreach ($route in $routes) {
    ...
    try {
        Remove-NetRoute -DestinationPrefix $network -NextHop 0.0.0.0 -InterfaceAlias $interfaceAlias -ErrorAction Stop -Confirm:$false

    }
    catch {
        Write-Warning "Не удалось удалить маршрут для $network ($_)"
    }
}
